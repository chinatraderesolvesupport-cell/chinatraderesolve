...
## Как работает фильтр v3.7.49

Монитор не ищет одну точную фразу. Он сопоставляет несколько групп признаков:

1. **Источник сделки:** Alibaba, 1688, Made-in-China, AliExpress, Китай, китайский поставщик/фабрика и эквиваленты на шести языках.
2. **Проблема:** неотправка, недоставка, брак, повреждение, неверный материал/размер/цвет, недостача, отказ в возврате, исчезновение продавца, отклонённый спор, сертификат или таможня.
3. **Просьба о помощи и сделка:** «что делать», «куда жаловаться», «как вернуть деньги», заказ, товар, предоплата, платёж и т. п.

Основное правило: уведомление создаётся при сочетании источника и конкретной проблемы. Название публичного чата может дать источник (например, «Закупки из Китая»), но сама проблема должна находиться в сообщении. Это снижает ложные уведомления.

Очевидный шум (вакансии, обучение, вебинары, криптосигналы, акции Alibaba) исключается. Дополнительные точные фразы по-прежнему можно задать через `TELEGRAM_MONITOR_KEYWORDS`, а исключения — через `TELEGRAM_MONITOR_EXCLUDE_KEYWORDS`.
