# ChinaTradeResolve Document AI v3.7.47

Version 3.7.47 broadens the local upload-guidance intent so natural questions about sending, attaching or uploading evidence receive the private-case workflow immediately, while retaining the Telegram recovery improvements from v3.7.46.
...
## Natural document-upload guidance in v3.7.47
...
- Requests to `chinatraderesolve.onrender.com` receive an HTTP 308 redirect to `https://chinatraderesolve.com`.
...
- Deployment and promotion instructions retain the v3.7.38 launch-gate changes and use current v3.7.47 release markers.
...
- A regression test verifies the responsive CSS; the current release markers are synchronised for `3.7.47`.
...
```bash
cd ChinaTradeResolve_Document_AI_v3.7.47
cp .env.example .env
