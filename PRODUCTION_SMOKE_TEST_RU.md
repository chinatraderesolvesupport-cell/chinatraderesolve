# Глубокая production-проверка ChinaTradeResolve v3.7.38

Версия 3.7.38 содержит администраторский сценарий `scripts/production_smoke_test.py`. Он запускается из **Render Shell** после успешного deploy и выполняет реальные проверки с действующими переменными среды.
