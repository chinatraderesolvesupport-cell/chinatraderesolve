# ChinaTradeResolve Document AI v3.7.50

Version 3.7.50 adds safe custom-domain activation, independently controlled search indexing, Yandex Webmaster verification, CSP-compatible Yandex Metrica support and direct IndexNow submission, while retaining the multilingual guides, Telegram monitor and all security, voice and document features.
...
## Context-aware Telegram opportunity detection in v3.7.50
...
- Uses a public chat title/username only as source context; the actual problem must still appear in the message.
- Covers Russian, English, Serbian, French, German and Spanish wording, including common inflections and conversational phrases.
- Avoids obvious noise such as jobs, courses, webinars, cryptocurrency signals and Alibaba stock discussion.
- Real alerts now clearly identify the source type, sender when Telegram provides it, message time, match reason and the public source link.
- 88 broad positive/negative phrase cases are covered in addition to the existing live-monitor tests.


## Natural document-upload guidance in v3.7.50
...
- The reply directs visitors to submit the short application and then use the private case page to upload up to twenty PDF or image files.
- Full documents and confidential order details are still kept out of the public chat.
- Regression tests cover the newly supported phrasing and preserve the existing scope guard.

## Telegram connection diagnostics and recovery in v3.7.46

- `/health` now reports a precise non-secret disconnect code, exception type and sanitised detail instead of the generic `connection_error`.
- It exposes connection attempts, successful connections, disconnect count, heartbeat time, next retry time and retry delay.
- A 45-second heartbeat detects stale MTProto connections even when no public messages arrive.
- Telethon uses explicit automatic reconnect, connection/request retries and bounded timeouts.
- Transient failures retry with short exponential backoff and jitter; revoked, duplicated or invalid sessions retry slowly and clearly identify the required action.
- Bot tokens, API hashes, session strings, URLs and IP addresses are redacted from public diagnostics.


## Canonical-domain redirect in v3.7.45

- Requests to `chinatraderesolve.onrender.com` receive an HTTP 301 для GET/HEAD и HTTP 308 для остальных методов redirect to `https://chinatraderesolve.com`.
- The complete path and query string are preserved, including private case links and campaign parameters.
- The target origin is fixed from the validated `PUBLIC_BASE_URL`; user input cannot choose a redirect destination.
- The canonical domain, localhost and unrelated hosts are not redirected.
- Redirect regression tests cover case URLs, query parameters and the normal `/health` response.
...
- GitHub CI restores executable permissions after checkout, so uploading through a ZIP no longer causes the `run_local.sh` regression test to fail.
- CI uses Node 24-compatible `actions/checkout@v6` and `actions/setup-python@v6`.
- Docker also restores script permissions explicitly instead of relying on archive metadata.
- `scripts/production_smoke_test.py` performs real readiness, application, document, email, OpenAI and voice checks from Render Shell.
- The test case is anonymised after the run, and a machine-readable `production_smoke_report.json` is saved.
- See `PRODUCTION_SMOKE_TEST_RU.md` for the exact production command.


## Public launch consistency in v3.7.38
...
- Deployment and promotion instructions retain the v3.7.38 launch-gate changes and use current v3.7.50 release markers.
...
- A regression test verifies the responsive CSS; the current release markers are synchronised for `3.7.50`.
...
```bash
cd ChinaTradeResolve_Document_AI_v3.7.50
cp .env.example .env
