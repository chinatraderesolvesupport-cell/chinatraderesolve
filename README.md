# ChinaTradeResolve Document AI v3.7.41

Version 3.7.41 fixes short contextual AI follow-ups, restores plugin-independent CI tests, and pins public email/case links to the canonical chinatraderesolve.com domain on the production Render service.
...
- Deployment and promotion instructions retain the v3.7.38 launch-gate changes and use current v3.7.41 release markers.
...
- A regression test verifies the responsive CSS; the current release markers are synchronised for `3.7.41`.
...
```bash
cd ChinaTradeResolve_Document_AI_v3.7.41
cp .env.example .env
