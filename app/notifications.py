...
def build_case_notifications(case: dict) -> list[dict[str, str]]:
    ...
    if language == "Russian":
        ...
        client_body = (f"Здравствуйте, {case['full_name']}!\n\nВаша бесплатная заявка ChinaTradeResolve получена.\n" f"Номер дела: {case['case_reference']}\nТекущий автоматический статус: {status_label}\n\n" f"Страница статуса: {status_url}\n\nОплата услуг сейчас не принимается. Сохраните приватную ссылку выше: через неё можно вернуться к статусу дела. Ключевые документы запрашиваются только после предварительного отбора заявки. Добровольная поддержка проекта не является оплатой услуг и не влияет на приоритет.\n")
    elif language == "French":
        client_subject = f"Demande ChinaTradeResolve {case['case_reference']}"
        client_body = (f"Bonjour {case['full_name']},\n\nVotre demande gratuite ChinaTradeResolve a bien été reçue.\n" f"Référence du dossier : {case['case_reference']}\nStatut automatique actuel : {status_label}\n\n" f"Page de statut : {status_url}\n\nLe paiement des services n’est pas accepté actuellement. Conservez le lien privé ci-dessus afin de pouvoir revenir au statut du dossier. Les documents essentiels ne sont demandés qu’après la présélection de la demande. Le soutien volontaire au projet ne constitue pas un paiement de services et n’influence pas la priorité.\n")
    elif language == "German":
        client_subject = f"ChinaTradeResolve-Antrag {case['case_reference']}"
        client_body = (f"Guten Tag {case['full_name']},\n\nIhr kostenloser ChinaTradeResolve-Antrag ist eingegangen.\n" f"Fallreferenz: {case['case_reference']}\nAktueller automatischer Status: {status_label}\n\n" f"Statusseite: {status_url}\n\nEine Bezahlung von Dienstleistungen wird derzeit nicht angenommen. Bewahren Sie den obigen privaten Link auf, um zur Statusseite des Falls zurückzukehren. Wichtige Dokumente werden erst nach der Vorprüfung des Antrags angefordert. Freiwillige Unterstützung des Projekts ist keine Bezahlung für Dienstleistungen und beeinflusst die Priorität nicht.\n")
    elif language == "Spanish":
        client_subject = f"Solicitud ChinaTradeResolve {case['case_reference']}"
        client_body = (f"Hola {case['full_name']},\n\nHemos recibido su solicitud gratuita de ChinaTradeResolve.\n" f"Referencia del caso: {case['case_reference']}\nEstado automático actual: {status_label}\n\n" f"Página de estado: {status_url}\n\nActualmente no se acepta el pago de servicios. Guarde el enlace privado anterior para volver a la página de estado del caso. Los documentos esenciales solo se solicitan después de la evaluación preliminar de la solicitud. El apoyo voluntario al proyecto no constituye el pago de servicios ni afecta a la prioridad.\n")
    elif language == "Serbian":
        client_subject = f"ChinaTradeResolve prijava {case['case_reference']}"
        client_body = (f"Poštovani/a {case['full_name']},\n\nVaša besplatna ChinaTradeResolve prijava je primljena.\n" f"Broj slučaja: {case['case_reference']}\nTrenutni automatski status: {status_label}\n\n" f"Stranica statusa: {status_url}\n\nPlaćanje usluga se trenutno ne prihvata. Sačuvajte privatni link iznad kako biste se vratili na stranicu statusa slučaja. Ključni dokumenti se traže tek nakon preliminarnog pregleda prijave. Dobrovoljna podrška projektu nije plaćanje usluge i ne utiče na prioritet.\n")
    else:
        client_subject = f"ChinaTradeResolve application {case['case_reference']}"
        client_body = (f"Hello {case['full_name']},\n\nYour ChinaTradeResolve free-access application has been received.\n" f"Case reference: {case['case_reference']}\nCurrent automated status: {status_label}\n\n" f"Status page: {status_url}\n\nPayment for services is not currently accepted. Save the private link above so you can return to the case status page. Key documents are requested only after the application has passed preliminary screening. Voluntary support for the project is not payment for services and does not affect priority.\n")
