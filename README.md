# ChinaTradeResolve Document AI v3.7.40

Version 3.7.40 adds an optional passive Telegram monitor for public supplier-dispute discussions, removes repeated unconditional startup messages, exposes non-secret monitor health, and keeps the existing production smoke-test and CI hardening.
...
## Passive Telegram monitor in v3.7.40
...
- Deployment and promotion instructions retain the v3.7.38 launch-gate changes and use current v3.7.40 release markers.
...
- A regression test verifies the responsive CSS; the current release markers are synchronised for `3.7.40`.
...
```bash
cd ChinaTradeResolve_Document_AI_v3.7.40
cp .env.example .env
